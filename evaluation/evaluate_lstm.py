...
# =========================
def evaluate_convlstm_model(model, loader, future_hours):
    """
    Evaluate ConvLSTM model for RMSE, SSIM, and Bias.
    ...
    with torch.no_grad():
        for x, y_true in tqdm(loader, desc=f"ConvLSTM {future_hours}h"):
    ...
    if batch_count == 0:
        raise RuntimeError("No batches processed in evaluate_convlstm_model.")
...
def main():
    ...
    for future_hours in [1, 2, 3, 4]:
        ...
        # -----------------------------
        model_path = f"models_new/conv_lstm_model_{future_hours}h.pth"
        if os.path.exists(model_path):
            ...
            frame_rmse, frame_ssim, frame_bias = evaluate_convlstm_model(model, loader, future_hours)
            ...
            print(f"  ConvLSTM {future_hours}h - Avg: RMSE={avg_rmse:.6f}, SSIM={avg_ssim:.4f}, Bias={avg_bias:.6f}")
        else:
            print(f"\n❌ ConvLSTM model for {future_hours}h not found, skipping...")
    ...
    for model_name in ["ConvLSTM"]:#["ConvGRU", "Persistence"]:
    ...
    # -----------------------------
    df = pd.DataFrame(all_results)
    output_csv = "evaluation_results_lstm_2019_roxi_0006.csv"
